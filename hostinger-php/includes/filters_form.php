<?php
/**
 * Renderiza a barra de filtros. $action é a URL base (imoveis.php ou
...
function render_filters_form(string $action, array $get, bool $showCityType, array $neighborhoods = []): void
{
    ...
    ?>
    <form method="get" action="<?= e($action) ?>" class="rounded-xl border border-brand-border bg-white p-4" id="filters-form">
      ...
      <div class="flex flex-wrap items-end gap-3">
        ...
        <button type="button" onclick="document.getElementById('more-filters').classList.toggle('hidden')" class="rounded-lg border border-brand-border px-3 py-2 text-sm font-medium hover:border-brand-primary">Mais filtros</button>
        <button type="submit" class="rounded-lg bg-brand-primary px-4 py-2 text-sm font-semibold text-white hover:bg-brand-primary-hover">Filtrar</button>
      </div>

      <div id="more-filters" class="mt-4 grid grid-cols-2 gap-3 border-t border-brand-border pt-4 sm:grid-cols-4">
        <?php
        $numFields = [
        ...
        foreach ($numFields as $field => $label): ?>
          <div>
            <label class="mb-1 block text-xs font-medium text-brand-text-secondary"><?= e($label) ?></label>
            <input type="number" min="0" name="<?= e($field) ?>" value="<?= $val($field) ?>" class="w-full rounded-lg border border-brand-border px-3 py-2 text-sm">
          </div>
        <?php endforeach; ?>
      </div>
    </form>
